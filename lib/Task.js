...
class Task {
  ...
  constructor(config) {
    ...
    let plugins = ['-cli', '-version'].concat(this.constructor.plugins)

    this.config = config
    this.name = name

    this.command = comanche(name, plugins)
  ...
  run(...args) {
    return new Promise((resolve) => {
      if (!this.hasStarted) {
        this.commandConfig = this.command.start()
        this.hasStarted = true
      }

      let batch = this._makeBatch(...args)
      let result = this.command.execute(batch)
